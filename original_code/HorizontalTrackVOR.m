...
% --- Plot the Capture Circles (5000 m radius) ---
...
for i = 1:size(beacons, 1)
    fill(beacons(i,2) + 5000*cos(theta), ...
         ...
         'LineStyle', '--', ...
         'FaceAlpha', 0.3, ...
         'HandleVisibility', 'off');   % Prevent legend clutter
end
...
% --- Label Each VOR with Its Number ---
for i = 1:size(beacons, 1)
    text(beacons(i,2) + 800, ...
         beacons(i,1) + 800, ...
         num2str(i), ...
         'FontSize', 11, ...
         'FontWeight', 'bold', ...
...
legend({'Flight Path', 'VOR Beacons', 'Start', 'End'}, ...
       'Location', 'northeastoutside');

% Adjust axes so circles aren't cut off
margin = 10000;
